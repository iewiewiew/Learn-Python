...
def extract_data_from_jmx(base_path, array):
    ...
    for jmx_path in array:
        jmx_file = base_path + 'testcase/gitee-pref-mirrors-' + jmx_path + '.jmx'
...
def save_report(base_path, array):
    ...
    for jmx_path in array:
        ...
        output_path = base_path + 'result/' + jmx_path + '/' + 'gitee_mirrors_report_{}_{}.xlsx'.format(jmx_path,
                                                                                            time.strftime("%Y%m%d",
                                                                                                          time.localtime()))
...
def merge_excel_files(base_path, array):
    ...
    df_first = pd.read_excel(base_path + 'result/' + array[0] + '/gitee_mirrors_report_' + array[0] + '_{}'.format(
    ...
    for file in array:
        df = pd.read_excel(base_path + 'result/' + file + '/gitee_mirrors_report_' + file + '_{}'.format(
            time.strftime("%Y%m%d", time.localtime())) + '.xlsx')
    ...
    output_path = base_path + 'result/' + 'gitee_mirrors_report_{}'.format(time.strftime("%Y%m%d", time.localtime())) + '.xlsx'
    df_final.to_excel(output_path, index=False)
    print("生成终版测试报告：" + output_path)


def merge_report(base_path):
    """合并两个报告"""
