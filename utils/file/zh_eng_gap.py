...
def demo():
    ...
    for root, dirs, files in os.walk(file_path):
        for _file_path in files:
            ...
            # if path.__contains__('.idea') or path.__contains__('.git') or path.__contains__('lib') or path.__contains__('tmp') or path.__contains__('venv'):

            conditions = [
                path.__contains__('.git'),
                path.__contains__('lib'),
                path.__contains__('tmp'),
                path.__contains__('venv')
            ]
            if any(conditions):
                return
            else:
